...
data <- data[,c('price', 'mileage', 'car_age', 'engine')]
data <- na.omit(data)
rownames(data) <- c()
...
kmeans_list <- list()
for(i in 1:8){
  kmeans_list[[i]] <- kmeans(data_scaled, centers = i, nstart = 100)
}
lasso <- glmnet(x=data_scaled[,2:4], y=data_scaled[,1])
xgb <- xgboost(data = data_scaled[,2:4], label = data_scaled[,1], nrounds = 100, verbose = FALSE)


#PCA is reversible
all(data == data.frame(apply(t(t(pca$x %*% t(pca$rotation)) * pca$scale + pca$center),2,round)))
...
#Example
seq_matrix(from = matrix(c(0,0,0,0,0,0),ncol=3),matrix(c(1,1,1,1,1,1),ncol=3),10,9)
...
ui <- fluidPage(
  ...
  tabPanel('Hand-made Component Analysis',
    sidebarPanel(
      ...
      sliderInput('pc1car_age', 'PC1 car_age:',
                  min = -1, max = 1,
                  value = pca$rotation['car_age',1], step = 0.01),
      sliderInput('pc2car_age', 'PC2 car_age:',
                  min = -1, max = 1,
                  value = pca$rotation['car_age',2], step = 0.01),
  ...
  tabPanel('What affects Toyota price?',
   mainPanel(
...
server <- function(input, output) {
  ...
  output$pca_animated <- renderPlot({
    ...
    pca_seq <- data_scaled %*% loadings_seq
    ...
    if(input$loading_names){
      for(i in 1:4){
        text(labels = c("Price", "car_age", "Mileage", "Engine capacity")[i], 
  ...
  output$position_info <- renderText({
    ...
    paste0(if(is.null(input$plot_click$x)) "Please select any point on the plot to see its coordinates and values below:\n",
           "Principal component 1 = ", input$plot_click$x,
           "\nPrincipal component 2 = ", input$plot_click$y,
           "\nClosest car's values:\nPrice = ", data$price[closest_point_id], 
           "\ncar_age = ", data$car_age[closest_point_id],
           '\nMileage = ', data$mileage[closest_point_id],
           '\nEngine capacity = ', data$engine[closest_point_id])
  })
  
  output$hca <- renderPlot({
    loadings_seq <- matrix(c(input$pc1price, input$pc1car_age, input$pc1mileage, input$pc1engine,
                             input$pc2price, input$pc2car_age, input$pc2mileage, input$pc2engine), ncol=2)
    pca_seq <- data_scaled %*% loadings_seq
