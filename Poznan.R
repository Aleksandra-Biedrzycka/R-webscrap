...
for ( i in 1:ileStron){
  urll<- paste0("https://www.otodom.pl/pl/oferty/sprzedaz/mieszkanie/poznan?areaMax=38&page=",i)
  remDr$navigate(urll)
  Sys.sleep(1)
...
for( l in 1:liczbaLinkow ){
  skip<-FALSE
  tryCatch(
    temp<-zrobWiersz(l,wektorLinkow,"Poznan",data,remDr=remDr),
    error=function(e){
      print(e)
      skip<<-TRUE
    }
  ...
  if ( !any(is.na(names(temp))) ){
    if( is.null(PoznanDF) )
      ...
      PoznanDF<-smartbind(PoznanDF,temp )
    }
  }
}
# "!r23_pjatK_23!"
...
dbWriteTable(con, "biedrzycka_Poznan", PoznanDF, append = FALSE,overwrite=TRUE)

dbListTables(con)
biedrzycka<- tbl(con,"biedrzycka_Poznan")
