...
for ( i in 1:ileStron){
  urll<- paste0("https://www.otodom.pl/pl/oferty/sprzedaz/mieszkanie/lodz?areaMax=38&page=",i)
  remDr$navigate(urll)
  Sys.sleep(1)
...
for( l in 1:liczbaLinkow ){
  skip<-FALSE
  tryCatch(
    temp<-zrobWiersz(l,wektorLinkow,"Lodz",data,remDr=remDr),
    error=function(e){
      print(e)
      skip<<-TRUE
    }
