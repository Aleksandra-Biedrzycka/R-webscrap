...
for ( i in 1:ileStron){
  urll<- paste0("https://www.otodom.pl/pl/oferty/sprzedaz/mieszkanie/warszawa?areaMax=38&page=",i)
  remDr$navigate(urll)
  Sys.sleep(1)
...
for( l in 1:liczbaLinkow ){
  skip<-FALSE
  tryCatch(
    temp<-zrobWiersz(l,wektorLinkow,"Warszawa",data,remDr=remDr),
    error=function(e){
      print(e)
      skip<<-TRUE
    }
  ...
  if ( !any(is.na(names(temp))) ){
    if( is.null(WarszawaDF) )
      WarszawaDF<-temp
    else{
      WarszawaDF<-smartbind(WarszawaDF,temp )
    }
  }
}
# "!r23_pjatK_23!"
...
dbWriteTable(con, "biedrzycka_Warszawa", WarszawaDF, append = FALSE,overwrite=TRUE)

dbListTables(con)
biedrzycka<- tbl(con,"biedrzycka_Warszawa")
